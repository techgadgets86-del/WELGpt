...
export default function InspirationPage() {
  ...
  const generateImageDataUrl = async () => {
    ...
    // Add skipFonts or use a fallback to prevent hanging on Google Fonts in Next.js
    const options = {
      quality: 1.0,
      ...
      fontEmbedCSS: '', // Prevents hanging on font fetches
    };
    
    try {
      // Run it once to "warm up" the cache (common iOS/Safari fix for missing images)
      await htmlToImage.toPng(cardRef.current, options);
      // Run it a second time for the actual render
      return await htmlToImage.toPng(cardRef.current, options);
    } catch (e) {
      console.error("htmlToImage failed:", e);
      alert("Failed to generate image. Please try another background mode.");
      return null;
    }
  };
    ...
    try {
      ...
      if (navigator.share) {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          ...
          // Fallback to share text/link only, then download image automatically
    ...
    <div className="max-w-4xl mx-auto relative z-10 pt-8 pb-32 sm:pb-20 w-full flex flex-col items-center justify-start min-h-full">
      ...
      >
        ...
        <button 
          onClick={handleShare}
          disabled={isGenerating}
          className="flex items-center justify-center w-full sm:w-auto gap-2 px-6 py-3.5 sm:py-3 rounded-full bg-gradient-to-r from-[#f09433] via-[#dc2743] to-[#bc1888] hover:opacity-90 text-white font-medium transition-opacity shadow-lg shadow-pink-500/20 disabled:opacity-50"
        >
          {isGenerating ? <Loader2 size={18} className="animate-spin" /> : <Share2 size={18} />}
          {isGenerating ? "Preparing..." : "Share to IG"}
        </button>
      </motion.div>
    </div>
  );
